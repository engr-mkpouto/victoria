...
async function main() {
  await mongoose.connect('mongodb+srv://Mkpouto17:Deron1992@face-detect.slbgbnb.mongodb.net/?retryWrites=true&w=majority');
...
const seedDB = async () => {
    ...
    for (let i = 0; i < 20; i++) {
        const random1000 = Math.floor(Math.random() * 30) + 1
        const r = Math.floor(Math.random() * 10000)
        const camp = new Student({
            email: `timaro${r}@gmail.com`,
            fname: `${name[random1000].state}`,
            lname: `${name[random1000].city}`,
            ...
            images: [
                {
                url: 'https://res.cloudinary.com/dlbofzsyr/image/upload/v1675891899/Oregano/y3dqkv9mogtqdyujtuni.png',
                filename: 'Oregano/y3dqkv9mogtqdyujtuni',
                }
            ],
            password:'Abc123@',
            reg: `REG-${r}`
